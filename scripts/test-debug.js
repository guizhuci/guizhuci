...
async function parseWordFile() {
  try {
    ...
    // 显示未匹配的题目
    const unmatched = questions.filter(q => !q.answer);
    if (unmatched.length > 0) {
      console.log('\n未匹配的题目:');
      unmatched.forEach((q, i) => {
        console.log(`  ${i + 1}. [${q.type}] index=${q.index} - ${q.question.substring(0, 40)}...`);
      });
    }
...
function parseQuestionsText(text) {
  ...
  parsedQuestions.forEach((q) => {
    if (q.type !== currentType) {
      currentType = q.type;
      typeStartIndex = q.index - 1;
    }

    const answer = parsedAnswers.get(`${currentType}_${q.index - typeStartIndex}`);
    console.log(`匹配: 题目[${q.type}_${q.index}] -> 查找答案[${currentType}_${q.index - typeStartIndex}] -> ${answer ? '✓' : '✗'}`);
...
function parseAnswersSection(text) {
  ...
  console.log('\n所有答案key:');
  answers.forEach((v, k) => {
    console.log(`  ${k}: ${v.answer.substring(0, 20)}...`);
  });
